...
export function Tournament8StartView(app: HTMLElement, state: any) {
  ...
  else if (!M.S1.winner && M.S1.p1 && M.S1.p2) { nextMatchRound = "S1"; nextMatchData = M.S1; currentRoundName = "Semifinales"; } 
  else if (!M.S2.winner && M.S2.p1 && M.S2.p2) { nextMatchRound = "S2"; nextMatchData = M.S2; currentRoundName = "Semifinales"; }
  ...
  function renderMatch(
    ...
    return `
      <div class="flex flex-col items-center gap-1 min-w-[100px] p-2 border border-gray-700 rounded-lg">
        ...
        ${
          match.winner
            ? `<p class="text-green-400 text-sm">Ganador: <b>${match.winner}</b></p>`
  ...
  app.innerHTML = `
    <div class="flex flex-col items-center w-full min-h-screen 
      ...
      <div class="backdrop-blur-md bg-black bg-opacity-40 
        ...
        <h3 class="text-lg font-bold text-poke-red border-b border-gray-600 pb-2">Cuartos de Final</h3>
        ...
        <h3 class="text-lg font-bold text-poke-red border-b border-t border-gray-600 pt-4 pb-2 mt-4">Semifinales</h3>
        ...
        ${!isTournamentComplete
            ? `<button id="startTournamentBtn"
                  ...
                  ▶️ Iniciar Torneo (${nextMatchRound ? t(nextMatchRound) : 'Esperando'})
              </button>`
            : ""
        }
